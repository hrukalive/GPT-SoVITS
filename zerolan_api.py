...
def _adapt_dict(req: dict):
    return TTS_Request(
        text=req['text'],
        text_lang=req["text_language"],
        ref_audio_path=req["refer_wav_path"],
        prompt_text=req["prompt_text"],
        prompt_lang=req["prompt_language"],
        media_type=req["audio_type"]
    )

from zerolan.data.pipeline.tts import TTSQuery

@APP.post("/tts/predict")
async def tts_post_endpoint(request: TTSQuery):
    req = request.dict()
    req = _adapt_dict(req).dict()
    req['streaming_mode'] = False
    return await tts_handle(req)

@APP.post("/tts/stream-predict")
async def stream_tts_post_endpoint(request: TTSQuery):
    req = request.dict()
    req = _adapt_dict(req).dict()
